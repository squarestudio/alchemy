window.Squarespace.onInitialize(Y, function() {
    ...
    if(welcomeJoinSection) {
        ...
        setTimeout(function () {
            ...
            // Checkbox logic
            const checkbox = newForm.querySelector('div.option input[type="checkbox"]');

            // Force pre-checked on load
            checkbox.defaultChecked = true;
            checkbox.checked = true;

            // Restore state from localStorage if available
            if (localStorage.getItem('my_checkbox_state') !== null) {
                checkbox.checked = localStorage.getItem('my_checkbox_state') === 'true';
            }

            // Persist state to localStorage on change
            checkbox.addEventListener('change', function () {
                localStorage.setItem('my_checkbox_state', checkbox.checked);
            });

            // Ensure it's checked again if the form tries to reset it
            newForm.addEventListener('submit', function () {
                checkbox.checked = true;
                localStorage.setItem('my_checkbox_state', 'true');
            });

            // MutationObserver to fight platform scripts that uncheck it
            const observer = new MutationObserver(() => {
                if (!checkbox.checked) {
                    checkbox.checked = true;
                    localStorage.setItem('my_checkbox_state', 'true');
            ...
            if (window.innerWidth > 1024) {
                newsletterButton.addEventListener("mouseenter", () => {
                    TweenMax.to("#joinBlock .form-block button[type=\"submit\"] .morph", 1, {
                        ...
                        morphSVG: "#joinBlock .form-block button[type=\"submit\"] .morph-hover",
                        yoyo: false
                    });
                });
                newsletterButton.addEventListener("mouseleave", () => {
                    TweenMax.to("#joinBlock .form-block button[type=\"submit\"] .morph", 1, {
                        ease: Power2.easeInOut,
                        morphSVG: "#joinBlock .form-block button[type=\"submit\"] .morph",
                        yoyo: false
                    });
                });
            }
